...
interface CompanyData {
  ...
  team: { id: number; name: string; role: string; slug: string; image?: string }[];
}
  ...
  return (
    <div className="overflow-hidden bg-white">
      ...
      <section className="relative h-[85vh] md:h-[90vh] flex items-center text-white bg-blue-950 overflow-hidden">
        <div className="absolute inset-0 z-0 bg-[url('/images/hero-kostin.png')] bg-cover bg-center opacity-40"></div>
              ...
              return (
                <div key={t.id} className="group bg-white p-8 md:p-10 rounded-[2.5rem] border border-blue-50 shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-2">
                  <div className="flex flex-col h-full">
                    ...
                    {/* CONTAINER FOTO BARU */}
                    <div className="relative w-14 h-14 bg-blue-50 text-blue-900 rounded-2xl flex items-center justify-center text-xl font-black mb-8 group-hover:bg-blue-600 group-hover:text-white transition-all duration-300 overflow-hidden shadow-inner">
                      {t.image ? (
                        <img
                          src={t.image}
                          alt={`Foto ${t.name}`}
